...
class Booking {
  ...
  render(element) {
    ...
    this.dom.checkboxes = this.dom.wrapper.querySelector(
      select.booking.checkboxes
    );
  }
  ...
  initActions() {
    ...
    this.dom.checkboxes.addEventListener("change", function (event) {
      ...
      if (
        clickedElem.tagName == "INPUT" &&
        clickedElem.getAttribute("type") &&
        clickedElem.getAttribute("name")
      ) {
  ...
  sendBooking() {
    ...
    this.makeBooked(
      ...
      this.hourPicker.value,
      this.hoursAmount.value,
      parseInt(this.selectedTable)
    );
    this.updateDOM();
  }
}
